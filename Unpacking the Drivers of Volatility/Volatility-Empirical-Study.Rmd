...
the volatility (often used as a statistical measure to assess the magnitude and 
frequency of price movements), of the stock market, with a specific focus on two prominent 
...
(as shown on the right, where the highlighted portions depict when recessions
occurred). Additionally, the analysis suggested that other factors
...
```{r side inflation plots, out.width="145%", fig.margin = TRUE, include=TRUE, echo=FALSE}
...
```{r side unemployment plots, out.width="145%", fig.margin = TRUE, include=TRUE, echo=FALSE}

knitr::include_graphics("UnemploymentRatePlot.png")

```

```{r side interest plots, out.width="145%", fig.margin = TRUE, include=TRUE, echo=FALSE}
...
\hspace{10mm}[1.2 Literature Review](#LiteratureReview) \hspace{10mm} 4 \
\hspace{10mm}[1.3 Purpose](#Purpose) \hspace{10mm} 5 \
\hspace{10mm}[1.4 Information](#Information) \hspace{10mm} 5 \
\
[2. Data Exploration](#DataExploration) \hspace{10mm} 7 \
\hspace{10mm}[2.1 Data Collection](#DataCollection) \hspace{10mm} 7 \
\hspace{10mm}[2.2 Data Description](#DataDescription) \hspace{10mm} 7 \
\hspace{10mm}[2.3 Exploratory Analysis](#ExploratoryAnalysis) \hspace{10mm} 8 \
\
[3. Data Analysis](#DataAnalysis) \hspace{10mm} 9 \
\hspace{10mm}[3.1 Method](#Method) \hspace{10mm} 9 \
\hspace{10mm}[3.2 Pre-Analysis](#PreAnalysis) \hspace{10mm} 11 \
\hspace{10mm}[3.3 Analysis](#Analysis) \hspace{10mm} 13 \
\hspace{10mm}\hspace{10mm}[3.3.1 Macroeconomic Data](#Macroeconomic)\hspace{10mm} 14 \
\hspace{10mm}[3.4 Observations](#Observations) \hspace{10mm} 18 \
\hspace{10mm}\hspace{10mm}[3.4.1 Gross Domestic Product](#GDP)\hspace{10mm} 18 \
\hspace{10mm}\hspace{10mm}[3.4.2 Debt-to-Equity Ratio](#DERatio)\hspace{10mm} 19 \
\hspace{10mm}\hspace{10mm}[3.4.3 Price-to-Earnings Ratio](#PERatio)\hspace{10mm} 20 \
\
[4. Discussion](#Discussion) \hspace{10mm} 22 \
\hspace{10mm}[4.1 Findings](#Findings) \hspace{10mm} 22 \
\hspace{10mm}[4.2 Limitations](#Limitations) \hspace{10mm} 22 \
\
[5. Conclusion](#Conclusion) \hspace{10mm} 24 \
\
[6. References](#References) \hspace{10mm} 25 \
\
[7. Appendix](#Appendix) \hspace{10mm} 26 \
\hspace{10mm}[7.1 Appendix A: Data Dictionary](#DataDictionary) \hspace{10mm} 26 \
\hspace{10mm}[7.2 Appendix B: Graphical Displays for Google](#GraphicalDisplaysforGoogle) \hspace{10mm} 30
...
findings using a distinct methodology (through the use of the GARCH model and 
linear regression model). Furthermore, it aims to contribute to the 
...
FRED (Federal Reserve Economic Data). I used Yahoo Finance to gather financial
...
the data, I used the search function to find the relevant company or stock, 
and then I extracted the data from the historical data or financial statements
...
\noindent I used FRED to collect economic data on a variety of indicators, including 
inflation rates, unemployment data, and GDP growth. I used the search function
to find the relevant economic indicator, and then I accessed the relevant data 
series.
\
\
...
volatility analysis, I used the adjusted close stock price. In addition,
...
```{r adjusted price trend apple, out.width="80%", fig.cap="Apple's Stock Price Time Series \\label{AppleStock}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
```{r adjusted price trend google, out.width="80%", fig.cap="Google's Stock Price Time Series \\label{GoogleStock}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
and the auto regressive conditional heteroskedasticity (ARCH) effect, was deemed 
necessary prior to modeling the log returns using a simple GARCH model. Volatility 
clustering is the tendency for volatility to persist for some time before 
transitioning to a different regime while the ARCH effect suggests that the 
volatility of a financial asset or security is not constant but changes over 
different periods (implies that the past volatility of an asset affects its 
future volatility). 
\
\

\noindent Through this analysis, the estimation of the conditional 
variance of the asset returns and the ability to forecast future volatility were enabled. 
Subsequently, the consideration was given to the incorporation of external
...
Nonetheless, for the study, the identification of a few factors that may 
have an impact on volatility was the aim. Hence, when the coefficients exhibited statistical 
...
I did this by analyzing and comparing their respective average residuals to 
the actual number.
...
ARCH effect. To ensure the correct application of the GARCH model, I, firstly, generated 
a plot of the log returns, derived from the computation of the differences
between the logarithmic adjusted closing stock prices of each company over time. 
This examination helped ascertain the presence of volatility clustering.
\
\

```{r volatility clustering apple, out.width="80%", fig.cap="Apple's Log Return Time Series \\label{AppleClustering}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}

ggplot(apple_stock) + geom_line(aes(x = Date, y = LogReturn), na.rm = TRUE)  + labs(title="Apple's Log Return") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) 

```

```{r volatility clustering google, out.width="80%", fig.cap="Google's Log Return Time Series \\label{GoogleClustering}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}

ggplot(google_stock) + geom_line(aes(x = Date, y = LogReturn), na.rm = TRUE)  + labs(title="Google's Log Return") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) 
```
...
```{r acf plots apple, fig.cap="Apple's ACF Plot \\label{AppleACF}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}

acf(apple_stock$LogReturn, main = "Series for Apple")

```

```{r acf plots google, fig.cap="Google's ACF Plot \\label{GoogleCF}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
\noindent Next, I conducted an examination to assess the fulfillment of the
...
coefficients that are statistically significant at 0.05, I obtained the subsequent 
...
\noindent At this stage, the study presents an analysis regarding various factors 
...
```{r volatility vs inflation rate apple, fig.cap="Relationship between Inflation Rate and the GARCH Model with Inflation Rate as an External Regressor for Apple \\label{InflationRate}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
par(mar=c(5, 5, 5, 5))
plot(apple_stock$Date, fit_apple_i@fit$sigma^2, type = 'l', col="red" , xlab = "Date", ylab = "", main = "Volatility and Inflation Time Series")
...
```{r volatility vs unemployment rate apple, fig.cap="Relationship between Unemployment Rate and the GARCH Model with Unemployment Rate as an External Regressor for Apple \\label{UnemploymentRate}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
par(mar=c(5, 5, 5, 5))
plot(apple_stock_e$Date, fit_apple_u@fit$sigma^2, type = 'l', col="red" , xlab = "Date", ylab = "", main = "Volatility and Unemployment Rate Time Series")
rect(as.Date("2020-01-01"), par("usr")[3], as.Date("2020-12-31"), par("usr")[4], col = rect_color, border = NA)
rect(as.Date("2007-01-01"), par("usr")[3], as.Date("2009-12-31"), par("usr")[4], col = rect_color, border = NA)
par(new=TRUE, xpd=TRUE)
...
```{r volatility vs interest rate apple, fig.cap="Relationship between Interest Rate and the GARCH Model with Interest Rate as an External Regressor for Apple \\label{InterestRate}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
par(mar=c(5, 5, 5, 5))
plot(apple_stock_e$Date, fit_apple_ir@fit$sigma^2, type = 'l', col="red" , xlab = "Date", ylab = "", main = "Volatility and Interest Rate Time Series")
rect(as.Date("2020-01-01"), par("usr")[3], as.Date("2020-12-31"), par("usr")[4], col = rect_color, border = NA)
rect(as.Date("2007-01-01"), par("usr")[3], as.Date("2009-12-31"), par("usr")[4], col = rect_color, border = NA)
par(new=TRUE, xpd=TRUE)
...
and the chosen macroeconomic variable, I also conducted a linear regression analysis.
...
```{r volatility vs inflation regression apple, fig.cap="Linear Relationship between Volatility and Inflation Rate for Apple \\label{Inflation}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
ggplot(df) + geom_point(aes(x = inflation, y = volatility)) + labs(x = "Inflation Rate",y = "Volatility", title = "Volatility vs Inflation Rate") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(formula = y ~ x, aes(x = inflation, y = volatility), method = "glm", se = FALSE, color = 'red')

```

```{r volatility vs unemployment rate regression apple, fig.cap="Linear Relationship between Volatility and Unemployment Rate for Apple \\label{Unemployment}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
ggplot(df) + geom_point(aes(x = unemployment_rate, y = volatility)) + labs(x = "Unemployment Rate", y = "Volatility", title = "Volatility vs Unemployment Rate") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(formula = y ~ x, aes(x = unemployment_rate, y = volatility), method = "lm", se = FALSE, color = 'red')

```

```{r volatility vs interest rate regression apple, fig.cap="Linear Relationship between Volatility and Interest Rate for Apple \\label{Interest}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
ggplot(df) + geom_point(aes(x = interest_rate, y = volatility)) + labs(x = "Interest Rate",y = "Volatility", title = "Volatility vs Interest Rate") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(formula = y ~ x, aes(x = interest_rate, y = volatility), method = "lm", se = FALSE, color = 'red')
...
relationship for all variables (e.g. Figure \ref{Interest}), the results of the linear regression test have 
...
exploration. Consequently, graphical displays are presented to identify 
...
is presented below in Figure \ref{GDP}. An intriguing observation emerged, wherein every instance of 
GDP decline coincides with a pronounced peak in volatility.

```{r gdp observation apple, fig.cap="Relationship between GDP and the GARCH Model with no External Regressor for Apple \\label{GDP}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
par(mar=c(5, 5, 5, 5))
plot(apple_stock$Date, fit_apple@fit$sigma^2, type = 'l', col="red" , xlab = "Date", ylab = "", main = "Volatility and GDP Time Series")
rect(as.Date("2020-01-01"), par("usr")[3], as.Date("2020-12-31"), par("usr")[4], col = rect_color, border = NA)
rect(as.Date("2007-01-01"), par("usr")[3], as.Date("2009-12-31"), par("usr")[4], col = rect_color, border = NA)
par(new=TRUE, xpd=TRUE)
...
\noindent The highlighted portions in Figure \ref{GDP} indicate significant declines 
...
\noindent Figure \ref{DE} below demonstrates a clear relationship between
...
```{r debt to equity ratio observation apple, fig.cap="Relationship between Debt-to-Equity and the GARCH Model with no External Regressor for Apple \\label{DE}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
par(mar=c(5, 5, 5, 5))
plot(apple_stock$Date, fit_apple@fit$sigma^2, type = 'l', col="red" , xlab = "Date", ylab = "", main = "Volatility and the D/E Ratio Time Series")
rect(as.Date("2020-01-01"), par("usr")[3], as.Date("2020-12-31"), par("usr")[4], col = rect_color, border = NA)
rect(as.Date("2007-01-01"), par("usr")[3], as.Date("2009-12-31"), par("usr")[4], col = rect_color, border = NA)
par(new=TRUE, xpd=TRUE)
...
\noindent In contrast to the previous financial metric, the upcoming plot  
exhibits alignment. This alignment is a result of the calculation incorporating 
...
\noindent Figure \ref{PE} suggests a positive relationship between
...
```{r price to earnings observation apple, fig.cap="Relationship between Price-to-Earnings and the GARCH Model with no External Regressor for Apple \\label{PE}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
par(mar=c(5, 5, 5, 5))
plot(apple_stock$Date, fit_apple@fit$sigma^2, type = 'l', col="red" , xlab = "Date", ylab = "", main = "Volatility and the P/E Ratio Time Series")
rect(as.Date("2020-01-01"), par("usr")[3], as.Date("2020-12-31"), par("usr")[4], col = rect_color, border = NA)
rect(as.Date("2007-01-01"), par("usr")[3], as.Date("2009-12-31"), par("usr")[4], col = rect_color, border = NA)
par(new=TRUE, xpd=TRUE)
...
\noindent In fact, during the initial phases in the study, I observed a noticeable 
pattern of missing data, which could be attributed to factors such as less widespread 
...
period, as indicated by the absence of comprehensive financial information for 
those years. 
\
\

\noindent Therefore, future research endeavors should aim to explore and expand
upon the findings by considering a more comprehensive range of factors, particularly
delving deeper into the company's financial aspects. 
\
\
...
the dynamics and trends within the stock market as a whole. The focus on only the
two companies with limited yearly information available also made it difficult to 
thoroughly analyze the impact of total revenue and other financial factors on 
volatility, such as stock price fluctuations and operating expenses. Consequently,
further investigation is necessary to explore and understand these relationships 
...
```{r volatility vs inflation google, fig.cap="Relationship between Inflation Rate and the GARCH Model with Inflation Rate as an External Regressor for Google \\label{InflationRategoogle}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
```{r volatility vs inflation regression google, fig.cap="Linear Relationship between Volatility and Inflation Rate for Google \\label{Inflationgoogle}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
ggplot(df) + geom_point(aes(x = inflation, y = volatility)) + labs(x = "Inflation Rate",y = "Volatility", title = "Volatility vs Inflation Rate") + theme_bw() + theme(plot.title = element_text(hjust = 0.5)) + geom_smooth(formula = y ~ x, aes(x = inflation, y = volatility), method = "lm", se = FALSE, color = 'red')

```

```{r volatility vs unemployment rate google, fig.cap="Relationship between Unemployment Rate and the GARCH Model with Unemployment Rate as an External Regressor for Google \\label{UnemploymentRategoogle}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
```{r volatility vs unemployment rate regression google, fig.cap="Linear Relationship between Volatility and Unemployment Rate for Google \\label{Unemploymentgoogle}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
```{r volatility vs interest rate google, fig.cap="Relationship between Interest Rate and the GARCH Model with Interest Rate as an External Regressor for Google \\label{InterestRategoogle}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
```{r volatility vs interest rate regression google, fig.cap="Linear Relationship between Volatility and Interest Rate for Google \\label{Interestgoogle}", fig.height=3, fig.width=5, fig.align='center', include=TRUE, echo=FALSE}
...
regression analysis were statistically insignificant. 
\
\
...
```{r gdp observation google, fig.cap="Relationship between GDP and the GARCH Model with no External Regressor for Google \\label{GDPgoogle}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
```{r debt to equity ratio observation google, fig.cap="Relationship between Debt-to-Equity and the GARCH Model with no External Regressor for Google \\label{DEgoogle}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
...
```{r price to earnings observation google, fig.cap="Relationship between Price-to-Earnings and the GARCH Model with no External Regressor for Google \\label{PEgoogle}", fig.height=4, fig.width=6, fig.align='center', include=TRUE, echo=FALSE}
